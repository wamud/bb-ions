...
def main():
    ...
    objects = [
        obj
        for obj in generate_documentation(obj=deltakit_stim, full_name="deltakit_stim", level=0)
        if all('[DEPRECATED]' not in line for line in obj.lines)
    ]

    print(f"# Deltakit-Stim {version} API Reference")
    print()
    if is_dev:
        print("*CAUTION*: this API reference is for the in-development version of deltakit-stim.")
...
'''.strip())
    ...
    for obj in objects:
        ...
        if len(obj.full_name.split('.')) > 2:
            ...
            print(f'# (at top-level in the deltakit_stim module)')
        print('\n'.join(obj.lines))
        print("```")
